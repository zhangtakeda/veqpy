...
namespace veqlib_python
{
    ...
    using veqlib_kernel_api::BenchGrid;
    using veqlib_kernel_api::BenchShape;
    using veqlib_kernel_api::BenchSource;
    ...
    using veqlib_kernel_api::bench_c_counts;
    using veqlib_kernel_api::bench_s_counts;

    using tensor::uninitialized;

    using PackedArrayView        = nb::ndarray<nb::numpy, const double, nb::shape<BenchShape::x_size>, nb::c_contig>;
    using MutablePackedArrayView = nb::ndarray<nb::numpy, double, nb::shape<BenchShape::x_size>, nb::c_contig>;
    ...
    {
        constexpr bool has_active_psin = BenchShape::slot_for_profile_id(BenchShape::psin_profile_id).optimized();
        ...
        {
            constexpr int psin0_index = BenchShape::coeff_index[BenchShape::psin_profile_id][0];
            ...
            else
            {
                ...
                const double target_scale = context.op.workspace.source_runtime.source_target_root_fields(
                                                source::root_psin, BenchGrid::radial_nodes - 1) -
                                            target_offset;
                ...
                for (size_t i = 0; i < BenchGrid::radial_nodes; ++i)
                {
                    ...
                    if (i == 0)
                        normalized = 0.0;
                    else if (i + 1 == BenchGrid::radial_nodes)
                        normalized = 1.0;

                    const double rho     = BenchGrid::nodes[i];
                    const double y       = BenchGrid::y[i];
                    const double rho2    = rho * rho;
                    const double base    = rho2;
                    const double basis   = rho2 * y;
                    const double weight  = BenchGrid::weights[i];
    ...
    void refine_cold_initial_state(SolveContext& context)
    {
        ...
        if constexpr (psin0_index >= 0)
        {
            ...
            context.input.x_scale =
                build_x_block_scale_vector<BenchShape>(context.input.x0, profile_params_for_case(context.input));
        }
    }
    ...
    nb::dict topology_metadata_dict(const CaseInput& input)
    {
        ...
        nb::dict profiles;
        using ProfileEvaluator  = profiles::ProfileEvaluator<BenchShape>;
        ...
        profiles["c_counts"]    = counts_list(bench_c_counts);
        profiles["s_counts"]    = counts_list(bench_s_counts);
    ...
    public:
        ...
        void set_case_json(const std::string& payload)
        {
            ...
            if (initial_policy_is_warm_clone(next_input.initial_policy_code))
            {
                ...
                next_input.x_scale =
                    build_x_block_scale_vector<BenchShape>(next_input.x0, profile_params_for_case(next_input));
            }
        ...
        {
            context_->raw_residual(std::span<const double, BenchShape::x_size>{x.data(), BenchShape::x_size},
