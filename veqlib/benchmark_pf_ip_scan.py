#!/usr/bin/env python3
"""PF/psin/uniform/Ip continuation-scan benchmark for the VEQlib artifact path."""
...
DEFAULT_OUTPUT = Path("/tmp/veqlib_pf_ip_scan.json")
...
from veqlib.benchmark_4case_compare import _benchmark_case_data  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    case_data = _benchmark_case_data(repeat=1, warmup=0)
    base_payload = json.loads(case_data.payload_json)
    base_ip = float(base_payload["constraints"]["scaled_Ip"])
    points = _scan_points(base_ip, points=args.points, relative_span=args.relative_span)
    payloads = [_payload_with_ip(base_payload, scaled_ip) for scaled_ip in points]

    cache = args.cache_root or Path(tempfile.mkdtemp(prefix="veqlib-pf-ip-scan-"))
    ...
    cold = _measure_policy(solver, payloads, repeat=args.repeat, warmup=args.warmup, policy="cold")
    warm = _measure_policy(
        solver, payloads, repeat=args.repeat, warmup=args.warmup, policy="warm-clone"
    )

    payload = {
        "schema": "veqlib.pf_ip_scan_continuation.v1",
        "case": case_data.name,
    ...
    print("| policy | wall median ms | internal median ms | total nfev | point nfev |")
    print("|---|---:|---:|---:|---|")
    for key in ("cold", "warm-clone"):
        item = payload["policies"][key]
        print(
            f"| {key} | {item['wall_ms']['median']:.6f} | "
            f"{item['internal_ms']['median']:.6f} | {item['total_nfev']['median']} | "
            f"{item['last_point_nfev']} |"
        )
    ratios = payload["ratios"]
    print(
        "warm/cold ratios: "
        f"wall={ratios['wall_median_warm_over_cold']:.3f} "
        f"internal={ratios['internal_median_warm_over_cold']:.3f} "
        f"nfev={ratios['nfev_median_warm_over_cold']:.3f}"
    )
    return 0
