...
from .numerics import make_quadrature
...
class VEQAdapter:
    ...
    def fill(self, plasma: Plasma) -> int:
        ...
        target = _target_nodes(self.topology)
        ...
        if self.topology.nodes == "explicit":
            source_nodes = coordinate_values
            pressure = pressure_values
            driver = _remap(driver_values[0], driver_values[1], source_nodes)
        else:
            source_nodes = target
            pressure = _remap(pressure_values[0], pressure_values[1], target)
            driver = _remap(driver_values[0], driver_values[1], target)
        count = int(source_nodes.size)
        capacity = int(self.topology.source_capacity)
        if count > capacity:
            raise ValueError(
                f"source input has {count} nodes but prepared capacity is {capacity}; "
                "construct a larger KernelTopology"
            )
...
def _target_nodes(topology: KernelTopology) -> np.ndarray:
    count = int(topology.source_capacity if topology.nodes == "explicit" else topology.sample_count)
    if topology.nodes == "uniform":
        return np.linspace(0.0, 1.0, count, dtype=np.float64)
    if topology.nodes == "grid":
        nodes, _ = make_quadrature(int(topology.Nr), scheme=topology.quadrature)
...
def _driver_values(plasma: Plasma, route: str, coordinate: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if route_key == "PP":
        ...
        if np.all(np.isfinite(current_q)):
            ...
            nodes = np.asarray(equilibrium.r, dtype=np.float64)
    else:
        if str(route_key) != "PF":
