...
/*     and calculate its norm. */
    ...
    iflag = evaluate(x, fvec, fjac, ldfjac, 1);
    *nfev = 1;
    if (iflag < 0) {
	goto TERMINATE;
    }
...
/*        calculate the jacobian matrix. */
        ...
        iflag = evaluate(x, fvec, fjac, ldfjac, 2);
        ++(*njev);
        if (iflag < 0) {
            goto TERMINATE;
        }
...
/*           evaluate the function at x + p and calculate its norm. */
            ...
            iflag = evaluate(wa2, wa4, fjac, ldfjac, 1);
            ++(*nfev);
            if (iflag < 0) {
                goto TERMINATE;
            }
