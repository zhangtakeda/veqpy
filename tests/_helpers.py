...
def pf_reference_profiles(psin: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    driver_input = (1.0 - beta0) * alpha_f * (np.exp(alpha_f * psin) - exp_af) / den_f
    pprime_input = beta0 * alpha_p * (np.exp(alpha_p * psin) - exp_ap) / den_p
    return driver_input.astype(np.float64), pprime_input.astype(np.float64)
