...
def _demo_equilibrium(*, b0: float = 3.0, ip: float = 3.0e6):
    ...
    p_psin, ff_psin = _profiles(source_axis)
    result = kernel.solve(
        boundary=boundary,
...
def _axis_even_r2_limit(values: np.ndarray, r: np.ndarray) -> np.ndarray:
    """Return a copy with the removable even axis value reconstructed."""

    result = np.asarray(values, dtype=np.float64).copy()
    if result.shape[0] >= 3 and abs(r[0]) < 1.0e-10:
        x0, x1, x2 = r[:3] ** 2
...
def test_uniform_equilibrium_exposes_neoclassical_geometry_profiles() -> None:
    ...
    np.testing.assert_allclose(
        equilibrium.epsilon,
        equilibrium.a * equilibrium.r / equilibrium.Rc,
    )
    assert equilibrium.h.shape == equilibrium.r.shape
    assert equilibrium.v.shape == equilibrium.r.shape
    assert equilibrium.kappa.shape == equilibrium.r.shape
...
def test_equilibrium_exposes_imas_toroidal_flux_coordinates() -> None:
    equilibrium = _uniform_demo_equilibrium()
    np.testing.assert_allclose(equilibrium.FF_psin, equilibrium.alpha2 * equilibrium.FF_psi)
    np.testing.assert_allclose(equilibrium.FF_r, equilibrium.FF_psin * equilibrium.psin_r)
    np.testing.assert_allclose(equilibrium.FF_r, equilibrium.FF_rho * equilibrium.rho_r)
    np.testing.assert_allclose(equilibrium.P_psin, equilibrium.alpha2 * equilibrium.P_psi)
    np.testing.assert_allclose(equilibrium.P_r, equilibrium.P_psin * equilibrium.psin_r)
    np.testing.assert_allclose(equilibrium.P_r, equilibrium.P_rho * equilibrium.rho_r)
    ...
    derivative = _axis_even_r2_limit(derivative, equilibrium.r)
    np.testing.assert_allclose(
    ...
    assert equilibrium.rho[-1] == pytest.approx(1.0, abs=3.0e-16)
...
def test_equilibrium_gm_series_matches_imas_flux_surface_averages() -> None:
    ...
    def surface_average(values: np.ndarray) -> np.ndarray:
        ...
        return _axis_even_r2_limit(result, equilibrium.r)
...
def test_signed_f_and_q_follow_the_fixed_cocos_contract(b0: float, ip: float) -> None:
    ...
    assert 0.0 < equilibrium.rho[0] < equilibrium.rho[-1] < 1.0
...
def test_pj2_jpara_reconstructs_imas_jtotal_with_gm1(b0: float) -> None:
    ...
    gm1_direct = _axis_even_r2_limit(gm1_direct, equilibrium.r)
    ...
    gm5_direct = _axis_even_r2_limit(gm5_direct, equilibrium.r)
    gm9_direct = _axis_even_r2_limit(gm9_direct, equilibrium.r)
...
def test_jtor_axis_is_the_conservative_itor_over_area_limit() -> None:
    equilibrium = _uniform_demo_equilibrium()
    r2_1 = equilibrium.r[1] ** 2
    r2_2 = equilibrium.r[2] ** 2
    ratio_1 = equilibrium.Itor[1] / equilibrium.S[1]
    ratio_2 = equilibrium.Itor[2] / equilibrium.S[2]
    expected = (r2_2 * ratio_1 - r2_1 * ratio_2) / (r2_2 - r2_1)

    assert equilibrium.r[0] == 0.0
