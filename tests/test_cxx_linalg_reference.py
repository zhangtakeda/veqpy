...
def _compile_reference_driver(
    ...
    *,
    ...
    label = "internal" if force_internal else "runtime-dispatch"
    output = tmp_path_factory.mktemp(f"cxx-linalg-reference-{label}") / "reference-driver"
    command = [
        compiler,
        "-std=c++20",
        "-O2",
    ...
    if force_internal:
        command.insert(3, "-DVEQPY_CXX_FORCE_INTERNAL_LINALG=1")
    else:
        command.append(str(BRIDGE_SOURCE))
    command.extend(["-o", str(output), "-llapacke", "-llapack", "-lopenblas"])
...
def linalg_reference_driver(tmp_path_factory: pytest.TempPathFactory) -> Path:
    return _compile_reference_driver(tmp_path_factory, force_internal=True)


@pytest.fixture(scope="module")
def linalg_runtime_dispatch_driver(tmp_path_factory: pytest.TempPathFactory) -> Path:
    return _compile_reference_driver(tmp_path_factory, force_internal=False)


def _assert_reference_rows(completed: subprocess.CompletedProcess[str]) -> None:
...
def test_fixed_size_linalg_matches_lapacke_on_reference_cases(
    ...
    _assert_reference_rows(completed)


def test_runtime_lapack_dispatch_matches_reference_cases(
    linalg_runtime_dispatch_driver: Path,
) -> None:
    completed = subprocess.run(
        [str(linalg_runtime_dispatch_driver)],
        check=True,
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
