...
def test_solver_config_user_defaults_aliases_and_validation() -> None:
    config = SolverConfig(
        method=None,
    ...
    assert SolverConfig(residual_normalization=None).residual_normalization == "fast"
...
def test_solver_result_is_an_owned_snapshot() -> None:
    x0 = np.array([1.0, 2.0])
    x = np.array([3.0, 4.0])
    result = SolverResult(
        x0=x0,
...
def test_solver_facade_lifecycle_and_problem_replacement() -> None:
